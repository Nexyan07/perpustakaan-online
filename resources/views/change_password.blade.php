<x-layout>
    ...
    <div class="flex justify-center items-center min-h-screen">
        <div x-data="{focus1: false, focus2: false, input1:'', input2:''}" class="w-96 mx-auto bg-gray-800 rounded-xl border">
            <a href="/validate-old-password" class="absolute inline-flex items-center text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 rounded-tl-xl text-sm px-5 py-4 text-center focus:ring-gray-900">
            ...
            <form action="{{ route('change.password') }}" method="POST" class="">
                @csrf
            <div class="flex flex-col items-center justify-center h-full mx-10">
                <div x-data="{show: false}" class="relative w-full mx-10 transition-all" :class="focus1 | input1 != '' ? 'mt-8' : 'mt-2'">
                    <input 
                    @focus="focus1 = true"
                    @blur="focus1 = false"
                    x-model='input1'
                    ...
                    <button x-show="input1 != ''" type="button" @click="show = !show" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-white" tabindex="-1">
            ...
            <div x-data="{show: false}" class="relative w-full mx-10 mb-5 transition-all" :class="focus2 | input2 != '' ? 'mt-8' : 'mt-4'">
                <input 
                  @focus="focus2 = true"
                  @blur="focus2 = false"
                  x-model='input2'
                  ...
                  class="border-0 border-b bg-gray-900 rounded border-white w-full text-sm px-3 py-2 peer placeholder-transparent 
                  focus:outline-none focus:ring-0 text-white">
                <button x-show="input2 != ''" type="button" @click="show = !show" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-white" tabindex="-1">
                    <img x-show="!show" src="{{ $BASEURL }}/img/assets/show.png" alt="" class="size-6">
                    <img x-show="show" src="{{ $BASEURL }}/img/assets/hide.png" alt="" class="size-6">
                </button>
